...
def _loads_jsonish(blob: str) -> Any:
    """Decode JSON, or the first value if Gemma wrapped it in commentary."""
    try:
...
def _looks_like_scene(obj: Any) -> bool:
    return isinstance(obj, dict) and (
        "beats" in obj or "scene" in obj or obj.get("refuse") is True
    )


def _coerce_json_object(payload: Any, *, depth: int = 0) -> Any:
    """Unwrap Gemma's quoted JSON, one-item arrays, and {result: scene} wrappers."""
    if depth > 5:
        return payload
    if isinstance(payload, str):
        ...
        try:
            ...
            return payload
        return _coerce_json_object(decoded, depth=depth + 1)
    if isinstance(payload, list):
        for item in payload:
            got = _coerce_json_object(item, depth=depth + 1)
            if isinstance(got, dict) and (_looks_like_scene(got) or "result" in got):
                return got
        for item in payload:
            got = _coerce_json_object(item, depth=depth + 1)
    ...
    if isinstance(payload, dict):
        if not _looks_like_scene(payload) and "result" in payload:
            inner = _coerce_json_object(payload.get("result"), depth=depth + 1)
