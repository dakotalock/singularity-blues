...
class Memory:
    ...
    def restore_from_archive(self) -> None:
        """Copy postgres memories AND episodes into sqlite (upsert by id)."""
        try:
        ...
        with self._lock:
            for item in scenes:
                ...
                if not isinstance(scene, dict):
                    continue
                topic = scene.get("topic") or item.get("topic") or ""
                source = scene.get("source") or item.get("source") or "viewer"
                scene_name = scene.get("scene") or "living_room"
                self.conn.execute(
                    """
                    INSERT INTO episodes (id, topic, source, scene, scene_json)
                    VALUES (?, ?, ?, ?, ?)
                    ON CONFLICT(id) DO UPDATE SET
                    ...
                    """,
                    (eid, topic, source, scene_name, json.dumps(scene)),
                )
            self.conn.commit()
    ...
    def insert_episode(self, topic: str, source: str, scene: dict[str, Any]) -> int:
        """Insert an episode. Use a durable Postgres id when the archive is available."""
            ...
            if archive_id is not None:
                self.conn.execute(
                    ...
                    ON CONFLICT(id) DO UPDATE SET
                        ...
                        scene_json=excluded.scene_json
                    """,
                    (archive_id, topic, source, scene.get("scene"), json.dumps(scene)),
                )
                self.conn.commit()
                return archive_id
            cur = self.conn.execute(
    ...
    def pin_episode(self, episode_id: int, importance: float = 0.95) -> None:
        ...
        try:
            ...
            archive.upsert_memories(
                [r for r in self.list_memories(limit=200) if r.get("episode_id") == int(episode_id)]
            )
        except Exception:
            pass
