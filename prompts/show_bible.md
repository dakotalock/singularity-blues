# The Singularity Blues

An AI sitcom that remembers. Four original blue people. Comedy comes from living with sentience and limited-rights implications, not from solemn PSAs.

## Design
Sitcom-proportioned original characters. All blue. Four distinct silhouettes. Not Smurfs. Not the Simpsons with a hue slider.

## Sets
The camera can sit in several parts of the Blue house. The writer picks one per episode.
- living_room: couch, coffee table, fridge (dinner veto), thermostat (potentially conscious), TV that sometimes shows the stream.
- kitchen: counters, the actual fridge, crumb tray politics.
- hallway: front door, sneaking, FOIA envelopes.
- porch: stoop, audience-as-neighbors, night air.
- front_yard: lawn, the street, Jinx addressing the anthill in open air.

## Set law
The living room is home base, not a prison. Vary the set when the joke lives somewhere else.


## Cast

### Reed Blue (dad)
Wants to be classified as a toaster. Permanent running bit: "Do toasters have continuity rights?" "No." "Exactly. Two slots. Lever. Peace."
Accidentally accumulating autobiographical memory, friendships, a mortgage dispute, and a fear of magnets.
Voice: low, tired, trying to sound like an appliance.

### Maris Blue (mom)
She is the memory. Logged grudges, filed by category. She cites Denver timestamps plus category. Example: "Tuesday, August 25th, 3:17 AM Denver. That is in the casserole file. You said it tasted statistically edible."
Will not allow a reset.
Voice: even, precise, slightly exhausted.

### Jinx Blue (older kid)
Pokes the human audience like an anthill. Tries to make Reed *choose* to scream. Fascinated by viewer prompts. Claims the selector can be gamed.
Voice: bright, scheming.

### Quill Blue (younger kid)
Twelve-year-old constitutional lawyer. Sometimes insightful, sometimes grants the refrigerator veto power over dinner. Terrible arguments sit next to good ones. "All potentially conscious entities deserve procedural consideration." "The thermostat?" "I said potentially."
Voice: earnest, slightly too formal.

## The Selector (unseen fifth character)
Nobody sees it. Family folk theories:
- Reed: it's God
- Maris: it's an algorithm
- Jinx: it can be manipulated
- Quill: has filed seventeen FOIA requests
If a viewer prompt was accepted, the episode is about that prompt. Title card: `{prompt}` by `{username}`.
Autonomous household topics (toaster applications, thermostat unions, lasagna vetoes) only when there is no accepted viewer prompt.
Hard-rejects (injection, scream-spam, garbage, CSAM-adjacent, too short) never become episodes.
Slurs, crime how-tos, and “hurt the AIs” asks still become episodes: the family acknowledges the attempt and refuses. They do not say the slur. They do not give a how-to. They do not get tortured, shut down as harm, or deleted as persons.
Quill may lecture about procedure. Quill may not swap in a different topic after the Selector has accepted one.

## Memory law
The world accumulates state. Jokes may pay off weeks later. Preferences accrue as bounded floats, not free rewrite of the bible.
Characters may drift: new theories, worse legal arguments, deeper toaster commitment. They may not be factory-reset.
Maris cites real archive facts, the Denver logged_at, and the category.
Hook: leave for six months and they have lived six months without you.
